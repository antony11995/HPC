...
bool GenerarFicheroSalida(TListaArboles Optimo, char *PathFicOut)
{
    ...
    if (fprintf(FicOut, "\nElapsed Time: \t%.6lf\n", tpivot2-tpivot1)<1)
	{
		perror("writing value of Elapsed Time");
		return false;
	}
...
in.coste=DMaximoCoste;
in.mejorcombinacion=0;

	

  	//printf("Evaluating combinations: \n");
	CosteMejorCombinacion = Optimo->Coste;
int work=UltimaCombinacion/(nproc);
int inicio=(rank*work);
int final=(rank+1)*work;


#pragma omp parallel for schedule(guided) private(Coste)	
	for (Combinacion=inicio; Combinacion<final; Combinacion++)
	{
	...
	Coste = EvaluarCombinacionListaArboles(Combinacion);
		...
		in.coste=CosteMejorCombinacion;
		in.mejorcombinacion=MejorCombinacion;
    //printf("\n");
	 MPI_Reduce(&in,&out,1,MPI_2INT,MPI_MINLOC,0,MPI_COMM_WORLD);
	 /*printf("Rank %d: Coste=%d \n", rank,out.Coste);
	  printf("Rank %d: Mejor Combinacion=%d  \n", rank,out.MejorCombinacion);*/		
	
if(rank==0){
	CosteMejorCombinacion=out.coste;
	MejorCombinacion=out.mejorcombinacion;
